...
/**
 * Credits to ChickenBones for this class, as found in the CodeChickenCore repository.
 */
public class DepLoader implements IFMLLoadingPlugin, IFMLCallHook
{
    ...
    public static class DepLoadInst
    {
        ...
        private void deleteMod(File mod)
        {
            ...
            // PISTON: simply try to delete on exit.
            // I don't know what the class loader thing is doing below, but higher java version
            // forbids access to internal classes, so I had to remove it
            ...
            if (!mod.delete())
            {
                ...
                // PISTON: forge is not going to like the System.exit below
                // just try later
        ...
        {
            // PISTON: my mcmod tool doesn't work well with things outside of the assets folder
            // so putting the file inside assets
            try
            {
                ZipFile zip = new ZipFile(file);
                ZipEntry e = zip.getEntry("assets/dependanciesGC.info");
                if (e == null)
                {
                    e = zip.getEntry("assets/dependenciesGC.info");
                }
                if (e != null)
                {
            ...
            {
                System.err.println("Failed to load assets/dependanciesGC.info from " + file.getName() + " as JSON");
                e.printStackTrace();
            }
        }
