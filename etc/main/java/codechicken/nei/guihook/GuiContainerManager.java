...
public class GuiContainerManager
{
    ...
    public static void drawItem(int i, int j, ItemStack itemstack, FontRenderer fontRenderer) {
        ...
        try {
            ...
            // PISTON: isDrawing is not visible
    ...
    public static Slot getSlotMouseOver(GuiContainer window) {
        ...
        // PISTON: use the inlined version
        return getSlotAtPosition(window, mousePos.x, mousePos.y);
    }

    // Inlining a bunch of stuff here because it's not accessible
    // this is equivalent to window.getSlotAtPosition(x, y);
    public static Slot getSlotAtPosition(GuiContainer window, int x, int y) {
        for (int k = 0; k < window.inventorySlots.inventorySlots.size(); ++k)
        {
            Slot slot = (Slot)window.inventorySlots.inventorySlots.get(k);

            // PISTON: inlining a bunch of stuff here because it's not accessible
            boolean isOver = false;
            {
                int k1 = window.guiLeft;
                int l1 = window.guiTop;
                int a5 = x - k1;
                int a6 = y - l1;
                isOver = a5 >= slot.xDisplayPosition - 1 && a5 < slot.xDisplayPosition + 16 + 1 && a6 >= slot.yDisplayPosition - 1 && a6 < slot.yDisplayPosition + 16 + 1;
            }
            if (isOver)
            {
                return slot;
