...
public class Assert {
	...
	public static void clientPlayerAPI(String modName) {
		if (!hasClientPlayerAPI)
			throw new RuntimeException(
					error(getPlayerAPIMessage(modName, true)));
	}

	public static void serverPlayerAPI(String modName) {
		if (!hasServerPlayerAPI)
			throw new RuntimeException(
					error(getPlayerAPIMessage(modName, false)));
	}

	private static String[] getPlayerAPIMessage(String modName, boolean client) {
		return getMessage(modName + " could not find the required API \""
				+ (client ? "Client" : "Server") + " Player\"!",
	...
	private static boolean hasAPI(String prefix) {
		String fullPrefix = "api.player."
				+ prefix.substring(0, 1).toLowerCase() + prefix.substring(1)
				+ "." + prefix + "Player";
		return Reflect.CheckClasses(Assert.class, new Name(fullPrefix + "API"),
				new Name(fullPrefix + "BaseSorter"));
	}
}